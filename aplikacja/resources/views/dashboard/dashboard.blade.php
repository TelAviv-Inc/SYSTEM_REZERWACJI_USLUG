...
<body class="font-sans bg-brand-bg text-brand-text min-h-screen flex flex-col justify-between"
    x-data="{ 
    ...
    <main class="flex-grow flex flex-col p-4">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-8">
            ...
            <!-- SIATKA KATEGORII (Dynamiczne kafelki z pętli) -->
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 justify-center items-stretch">
                @foreach ($categories as $category)
                    <div @click.prevent="selectedCategory = '{{ $category->name }}'" class="cursor-pointer transition-transform hover:-translate-y-1">
            ...
            <!-- SZCZEGÓŁOWE USŁUGI DLA WYBRANEJ KATEGORII -->
            <div x-show="selectedCategory !== null" x-cloak>
                <div class="flex justify-end mb-2">
                    <button @click="selectedCategory = null" class="text-xs text-brand-muted hover:text-brand-navy font-semibold">
                        Zamknij widok &times;
                    </button>
                </div>
    
                @include('dashboard.categories.category-view')
            </div>
    ...
    <x-modal name="booking-modal" maxWidth="md">
        <div class="p-6 space-y-6">
            <div class="flex items-center justify-between border-b border-brand-border pb-3">
                ...
                <button @click="$dispatch('close-modal', 'booking-modal')" class="text-brand-muted hover:text-brand-navy text-xl font-bold">&times;</button>
            </div>
            ...
            <form action="#" method="POST" class="space-y-4" @submit.prevent="$dispatch('close-modal', 'booking-modal')">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-brand-navy mb-1">Wybierz datę</label>
                    <input type="date" x-model="selectedDate" class="w-full text-sm border-brand-border rounded-md shadow-sm focus:border-brand-accent focus:ring-brand-accent" required>
                </div>
                ...
                <div class="pt-4 border-t border-brand-border flex items-center justify-end gap-3">
                    <button type="button" @click="$dispatch('close-modal', 'booking-modal')" class="px-4 py-2 text-xs font-semibold text-brand-navy bg-white border border-brand-border rounded-md hover:bg-brand-light-bg">
                        Anuluj
                    </button>
                    <button type="submit" class="px-4 py-2 text-xs font-semibold text-white bg-brand-accent hover:bg-brand-hover rounded-md shadow-sm">
